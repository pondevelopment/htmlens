...
pub fn validate_manifest(json_content: &str) -> ManifestValidation {
    ...
    if !valid_auth_types.contains(&manifest.auth.auth_type.as_str()) {
        validation.add_issue(
            "auth.type",
            &format!(
                "Invalid auth type '{}' (must be: none, user_http, or service_http)",
                manifest.auth.auth_type
            ),
        );
    }
    ...
    if manifest.api.api_type != "openapi" {
        validation.add_issue(
            "api.type",
            &format!(
                "Invalid API type '{}' (currently only 'openapi' is supported)",
                manifest.api.api_type
            ),
        );
    }
...
mod tests {
    ...
    fn test_valid_manifest() {
        ...
        assert!(
            validation.valid,
            "Expected valid manifest but got issues: {:?}",
            validation.issues
        );

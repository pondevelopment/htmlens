...
pub fn parse_robots_txt(content: &str) -> RobotsTxtAnalysis {
            ...
            "user-agent" => {
                ...
                if !current_agents.is_empty() {
                    for agent in &current_agents {
                        analysis.agent_rules.insert(agent.clone(), current_rules.clone());
                    }
                }
    ...
    if let Some(wildcard) = analysis.agent_rules.get("*") {
        if wildcard.blocks_all {
            analysis.issues.push("Warning: All bots blocked with 'Disallow: /'".to_string());
        }
    }
...
fn get_applicable_rules(crawler: &str, agent_rules: &HashMap<String, AgentRules>) -> Option<String> {
    // Check for exact match
    if let Some(rules) = agent_rules.get(crawler) {
...
pub fn is_path_allowed(path: &str, agent: &str, analysis: &RobotsTxtAnalysis) -> bool {
    let rules = analysis.agent_rules.get(agent)
...
"#;
        ...
        let gptbot_status = analysis.ai_crawler_status
            .iter()
